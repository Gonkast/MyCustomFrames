...
function ns.RefreshRaid()
    ...
    -- FIX (2026-07-24, "la solucion de direccion solo se ve en el preview de
    -- Lock, los raid frames REALES no la aplican"): el preview (LayoutGhosts)
    -- replica a mano el algoritmo real de SecureGroupHeaderTemplate (ver doc
    -- referenciada en GetRelPoint mas abajo) y por eso necesitaba el signo
    -- (LEFT=+1/RIGHT=-1) para que growPoint=RIGHT crezca al lado contrario.
    -- El header REAL usa el MISMO mecanismo primitivo point/xOffset/yOffset
    -- de Blizzard (SetPoint crudo, sin flip automatico segun el point elegido
    -- -- a diferencia de columnAnchorPoint/columnSpacing, que SI son un API
    -- de mas alto nivel con signo resuelto internamente), asi que necesita
    -- exactamente el mismo signo o crece siempre hacia el mismo lado sin
    -- importar growPoint -- quedaba "arreglado" solo en el preview.
    local xMult = (cfg.growPoint == "RIGHT") and -1 or 1
    raidHeader:SetAttribute("point", cfg.growPoint)
    raidHeader:SetAttribute("xOffset", cfg.growXOffset * xMult)
...
local function LayoutGhosts()
    ...
    -- o el spacing"): xMult/yMult venian de GetRelPoint con un SIGNO a
    -- proposito (LEFT=+1/RIGHT=-1, TOP=-1/BOTTOM=+1 -- ver la funcion arriba)
    -- para que growPoint=RIGHT crezca hacia el lado contrario que LEFT. El
    -- math.abs() de aca lo BORRABA -- RIGHT terminaba anclando cada ghost
    -- igual que LEFT (mismo lado), mientras el PRIMER ghost si quedaba pineado
    -- a la esquina correcta (esa linea no usa xMult) -- el resto del bloque
    -- crecia para el lado equivocado relativo a esa esquina y se salia del
    -- outline (que SI mide el tamaño total con el signo real, ver
    -- ComputeGridSize). Mismo bug para columnAnchorPoint TOP/BOTTOM.
...
raidEvents:SetScript("OnEvent", function(self, event, arg1)
    ...
    -- las preview en lock"): ns.UpdateRaidGhosts() (oculta el preview si hay
    -- members reales visibles) solo se llamaba al togglear Lock o desde el
    -- menu -- si el roster real cambia MIENTRAS ya estas en Lock (unirte a un
    -- raid, por ejemplo), UpdateRaidDrivers de arriba ni se molesta en tocar
