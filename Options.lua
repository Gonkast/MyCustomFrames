...
local function BuildPanel()
    ...
    local minimapSecList = {
        ...
        -- Pedido del usuario 2026-07-24: controles para el sistema de hover
        -- de botones de addons (MinimapButtons.lua) -- settings GLOBALES
        -- (db.minimapbuttons, singleton propio como Party/Arena Auras), asi
        -- que sus widgets usan getTbl/onChange en vez de getP()/OnEdit,
        -- mismo patron. Vive como pestaña ACA (no como grupo propio del
        -- sidebar) porque el usuario lo pidio "en el menu del minimapa".
        { key = "mn_buttons",  label = "Buttons" },
    }
    ...
    do
        ...
        HIDEGRP.powerHidden[#HIDEGRP.powerHidden + 1] = MakeCheckbox(f, "Percent red on low HP", "percentLowHealthColor", L, -108)
        HIDEGRP.powerHidden[#HIDEGRP.powerHidden + 1] = MakeSlider(f, "Percent red below %", 1, 99, 1, "percentLowHealthThreshold", L, -150)
    ...
    do
        ...
        -- Pedido del usuario 2026-07-24 ("dale paridad de menu a Top Widget"):
        -- Strata/Anchor no tenian control aca (solo offset/scale) -- mismo
        -- patron que la pestaña General de units/portraits (MakeCycle
        -- Strata + MakeEditBox Anchor), pero armados a mano porque esos
        -- helpers genericos usan getP()/OnEdit fijos (perfil actual via
        -- CurrentProfile) y TopWidget vive en db.topwidget, fuera de ese
        -- sistema (settings GLOBALES, como Party/Arena Auras y ahora
        -- Minimap Buttons).
        ...
        local anchorBox = CreateFrame("EditBox", nil, topWidgetGroup, "InputBoxTemplate")
    ...
    -- Minimap / Buttons (2026-07-24, pedido del usuario): controles del sistema de
    -- hover de botones de addons (MinimapButtons.lua). SINGLETON con settings
    -- GLOBALES (db.minimapbuttons, no db.minimap) -- mismo patron getTbl/onChange
    ...
    do
        ...
        note:SetText("Collects other addons' minimap icons and reveals them on hover. The trigger " ..
        ...
        MakeSlider(f, "Icons per row/column", 1, 10, 1, "perRow", L, -208, MMBDB, RefreshMMB)

        MakeSlider(f, "Row spacing", 0, 20, 1, "rowSpacing", R, -154, MMBDB, RefreshMMB)
        MakeSlider(f, "Column spacing", 0, 20, 1, "colSpacing", R, -208, MMBDB, RefreshMMB)

        -- Pedido del usuario 2026-07-24 ("dale paridad de menu"): Strata/Anchor,
        ...
        local anchorBox = CreateFrame("EditBox", nil, f, "InputBoxTemplate")
        anchorBox:SetSize(200, 20)
